...
전용 club head/grip 검출 모델도 같은 원칙을 따른다. repo에는 `GOLFLOG_CLUB_DETECTOR_COMMAND` 실행 계약과 fallback 코드만 두고, 모델 가중치, 학습 데이터, inference runtime artifact는 Git 밖에 둔다.

```bash
/Volumes/X31/golflog-data/models/club_detector/
```
