...
```text
영상 파일은 선택하지만 실제 pose 모델 분석은 아직 수행하지 않는다.
현재 결과는 mock analysis result다.
```

### 15.2 현재 GolfLog repo 기준 권장 구조
...
GolfLog/
 ...
 │   │   └─ swing-ai/
 │   │       ├─ VideoUploader.tsx
 │   │       ├─ SwingVideoPlayer.tsx
 │   │       ├─ SkeletonOverlay.tsx
 │   │       ├─ ClubPathOverlay.tsx
 │   │       ├─ PhaseTimeline.tsx
 │   │       └─ AnalysisReportPanel.tsx
 │   └─ data/
 │       └─ schema.ts
 │
 ├─ server/
...
Phase 1의 목표는 “완성형 AI 코치”가 아니라, mock 결과를 실제 영상 기반 2D skeleton 결과로 교체하는 것이다.
...
→ POST /api/analysis
→ Node가 영상을 server-data/videos/{analysisId}.mp4에 저장
...
→ server-data/analysis/{analysisId}.json 저장
...
```text
POST /api/analysis에 실제 mp4 파일 업로드 가능
server-data/videos/{analysisId}.mp4 생성
server-data/analysis/{analysisId}.json 생성
...
```text
server-data/videos/{analysisId}.mp4
server-data/analysis/{analysisId}.json
```
...
```json
{
  ...
  "videoPath": "server-data/videos/ana_20260430_xxxxxx.mp4",
  "resultPath": "server-data/analysis/ana_20260430_xxxxxx.json",
...
python workers/pose/analyze_pose.py \
  --video server-data/videos/ana_001.mp4 \
  --out server-data/analysis/ana_001.worker.json \
  --model mediapipe \
  --view-angle side \
  --club-type driver \
  --dominant-hand right
...
Phase 2에서는 처음부터 SwingNet을 붙이지 말고, 실제 pose/club trajectory 기반 heuristic을 먼저 구현한다.
...
```text
feat: add local pose worker pipeline for swing analysis
```

포함 작업:

```text
- save uploaded swing videos locally
- add workers/pose MediaPipe baseline
- run Python pose worker from Node analysis API
- persist normalized analysis result JSON
- render real skeleton overlay on swing video
```
...
python workers/pose/analyze_pose.py --video <sample.mp4> --out <out.json>
```
