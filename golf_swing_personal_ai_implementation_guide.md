...
- 분석 품질 metadata(`analysisQuality`)와 coach recommendation 확장 필드 추가
...
- worker `debug` 정보를 `analysisQuality`로 승격
- 2D proxy metric 계산: pose confidence, club detection rate, head/pelvis sway, address spine angle, left arm bend, shoulder/hip turn, tempo
- fallback synthetic pose 결과는 정상 score처럼 보이지 않게 처리
...
- 분석 품질 badge, fallback warning, capture guide 표시
- confidence-aware skeleton overlay
- coach-style recommendation card와 `View phase` seek 연결
...
fallback/mock 결과가 정상 점수처럼 표시되지 않음
```
...
현재 metric은 단일 카메라 2D proxy이며 정밀 biomechanical 진단값으로 표현하지 않는다.
```

### Current Priority: Trustworthy 2D Analysis UX

현재 다음 우선순위는 3D/IK/ghost swing이 아니라 2D 분석 결과를 사용자가 신뢰 가능한 방식으로 이해하도록 만드는 것이다.
...
- recommendation-to-phase navigation
- capture guide
- 핵심 swing-ai component split
- 2D proxy metric baseline

계속 보강:
- 실제 본인 스윙 영상 기준 품질 QA
- low-confidence/club detection 실패 구간의 UI 보정 흐름
- 전용 club 모델 연결 및 OpenCV baseline 대비 평가
```
...
GolfLog/
 ...
 │   ├─ components/
 │   │   └─ swing-ai/
 │   │       ├─ AnalysisQualityBadge.tsx
 │   │       ├─ CaptureGuideCard.tsx
...
현재는 `SwingAiPage.tsx`와 `server/server.js`에 기능이 아직 일부 집중되어 있다. 단, 분석 품질 badge, capture guide, recommendation cards, skeleton overlay는 `src/components/swing-ai/`로 분리했고, 2D proxy metric 계산은 `server/analysis/metrics.js`로 분리했다.
...
```text
완료:
src/components/swing-ai/AnalysisQualityBadge.tsx
src/components/swing-ai/CaptureGuideCard.tsx
src/components/swing-ai/RecommendationCards.tsx
src/components/swing-ai/SkeletonOverlay.tsx

다음 분리 후보:
src/components/swing-ai/SwingInputCard.tsx
src/components/swing-ai/SwingHistoryCard.tsx
src/components/swing-ai/SwingVideoOverlay.tsx
src/components/swing-ai/PhaseTimeline.tsx
src/components/swing-ai/ScoreGrid.tsx
src/components/swing-ai/BiomechanicsSummary.tsx
src/components/swing-ai/PhaseEditor.tsx
src/components/swing-ai/ClubCorrectionEditor.tsx
```
...
0.4 <= confidence < 0.7인 keypoint/bone은 낮은 opacity로 표시한다.
frame 평균 confidence가 낮으면 `Low pose confidence` label을 표시한다.
...
현재는 단일 카메라 2D 기반 proxy metric을 먼저 계산한다. 이 값은 정밀한 생체역학 진단값이 아니라 UI/추천 엔진을 안정화하기 위한 상대 지표다.
...
head_sway_proxy
pelvis_sway_proxy
address_spine_angle_proxy
left_arm_bend_at_top_proxy
shoulder_turn_proxy
hip_turn_proxy
tempo_ratio
```
...
```text
현재: server/analysis/metrics.js
후속: workers/pose/metrics.py 또는 packages/biomechanics
...
```json
{
  ...
  "club_detection_rate": 0.41,
  "head_sway_proxy": 4.8,
  "pelvis_sway_proxy": 3.1,
  "address_spine_angle_proxy": 12.4,
  "left_arm_bend_at_top_proxy": 8.7,
  "shoulder_turn_proxy": 18.2,
  "hip_turn_proxy": 7.1,
  "tempo_ratio": 3.1
}
```
...
```json
{
  ...
  "title": "백스윙 중 머리 이동이 큽니다",
  "evidenceMetrics": {
    "head_sway_proxy": "8.4% frame width"
  },
...
현재 local pose worker pipeline과 Trustworthy 2D Analysis UX baseline은 완료됐다. 다음 커밋 후보:

```text
feat: validate swing analysis with personal real swing video
```

포함 작업:

```text
- 실제 본인 스윙 영상으로 pose confidence, club detection rate, dropped frame 확인
- capture guide 기준으로 재촬영 전/후 분석 품질 비교
- phase/club 수동 보정 workflow 확인
- 분석 결과 JSON과 영상은 Git 밖 로컬 데이터 경로에만 보관
- 이후 GolfDB/SwingNet 계열 phase 모델 검토와 전용 club model 연결로 이동
```
